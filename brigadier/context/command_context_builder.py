...
class CommandContextBuilder:
    ...
    def copy(self):
        copy = CommandContextBuilder(
            self.dispatcher, self.source, self.root_node, self.range.get_start()
        )
    ...
    def build(self, str_input):
        return CommandContext(
            ...
            self.nodes,
            self.range,
            None if self.child is None else self.child.build(str_input),
            self.modifier,
            self.forks,
        )
            ...
            if self.range.get_end() < cursor:
                if self.child is not None:
                    ...
                    return SuggestionContext(
                        last.get_node(), last.get_range().get_end() + 1
                    )
                else:
                ...
                for node in self.nodes:
                    ...
                    if (
                        node_range.get_start() <= cursor
                        and cursor <= node_range.get_end()
                    ):
