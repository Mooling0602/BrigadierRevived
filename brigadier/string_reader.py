...
class StringReader:
    ...
    def read_int(self):
        ...
        try:
            ...
            self.cursor = start
            raise BuiltInExceptions.reader_invalid_int().create_with_context(
                self, number
            )
    ...
    def read_quoted_string(self):
        ...
        if not self.is_quoted_string_start(next_char):
            raise BuiltInExceptions.reader_expected_start_of_quote().create_with_context(
                self
            )
        ...
        while self.can_read():
            ...
            if escaped:
                if char == terminator or char == self.SYNTAX_ESCAPE:
                    ...
                    raise BuiltInExceptions.reader_invalid_escape().create_with_context(
                        self, str(char)
                    )
    ...
    def read_boolean(self):
        ...
        if value == "true":
            ...
            self.cursor = start
            raise BuiltInExceptions.reader_invalid_bool().create_with_context(
                self, value
            )

    def expect(self, char):
        if not self.can_read() or self.peek() != char:
            raise BuiltInExceptions.reader_expected_symbol().create_with_context(
                self, str(char)
            )
