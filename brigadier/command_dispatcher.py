...
class CommandDispatcher:
    ...
    def execute_preparsed(self, parse):
        if parse.get_reader().can_read():
            if len(parse.get_exceptions()) == 1:
                ...
                raise BuiltInExceptions.dispatcher_unknown_command().create_with_context(
                    parse.get_reader()
                )
            else:
                raise BuiltInExceptions.dispatcher_unknown_argument().create_with_context(
                    parse.get_reader()
                )
    ...
    def parse_nodes(self, node, original_reader, context_so_far):
        ...
        for child in node.get_relevant_nodes(original_reader):
            ...
            try:
                ...
                # TODO: check if this one's appropriate
                    ...
                    raise BuiltInExceptions.dispatcher_parse_expection().create_with_context(
                        reader, e
                    )

                if reader.can_read():
                    if reader.peek() != ARGUMENT_SEPARATOR_CHAR:
                        raise BuiltInExceptions.dispatcher_expected_argument_separator().create_with_context(
                            reader
                        )
            except CommandSyntaxException as e:
                if errors is None:
            ...
            if reader.can_read(2 if child.get_redirect() is None else 1):
                ...
                if child.get_redirect() is not None:
                    child_context = CommandContextBuilder(
                        self, source, child.get_redirect(), reader.get_cursor()
                    )
                    parse = self.parse_nodes(
                        child.get_redirect(), reader, child_context
                    )
                    context.with_child(parse.get_context())
                    return ParseResult(
                        context, parse.get_reader(), parse.get_exceptions()
                    )
                else:
        ...
        if potentials:
            ...
            return potentials[0]
        return ParseResult(
            context_so_far, original_reader, {} if errors is None else errors
        )
    ...
    def __get_all_usage(self, node, source, result, prefix, restricted):
        ...
        if node.get_redirect() is not None:
            redirect = (
                "..."
                if node.get_redirect() == self.root
                else f"-> {node.get_redirect().get_usage_text()}"
            )
            result.append(
                node.get_usage_text() + ARGUMENT_SEPARATOR + redirect
                if not prefix
                else prefix + ARGUMENT_SEPARATOR + redirect
            )
        elif node.get_children():
            for child in node.get_children():
                self.__get_all_usage(
                    child,
                    source,
                    result,
    ...
    def get_smart_usage(self, node, source, optional, deep):
        ...
        this = (
            USAGE_OPTIONAL_OPEN + node.get_usage_text() + USAGE_OPTIONAL_CLOSE
            if optional
            else node.get_usage_text()
        )
        ...
        if not deep:
            if node.get_redirect() is not None:
                ...
                children = list(
                    filter(lambda c: c.can_use(source), node.get_children())
                )
                if len(children) == 1:
                    usage = self.__get_smart_usage(
                        children[0], source, child_optional, child_optional
                    )
                    ...
                    for child in children:
                        usage = self.__get_smart_usage(
                            child, source, child_optional, True
                        )
                        if usage is not None:
    ...
    async def get_completion_suggestions(self, parse, cursor=None):
        ...
        for node in parent.get_children():
            ...
            try:
                future = node.list_suggestions(
                    context.build(truncated_input),
                    SuggestionsBuilder(truncated_input, start),
                )
